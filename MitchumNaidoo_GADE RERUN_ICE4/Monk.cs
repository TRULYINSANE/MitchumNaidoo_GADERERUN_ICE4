...
namespace MitchumNaidoo_GADE_RERUN_ICE4
{
    public class Monk : Hero
    {
        ...
        //constructor
        public Monk(int hp, int attack, int crit, int defense, string name, string battlecry) : base (hp ,attack, crit, defense, name, battlecry)
        {
            Hp = hp = SetHp();
        ...
        //methords
        public static void CreateMonk(int hp, int attack, int crit, int defense, string name, string battlecry)
        {
            SetHp();
            SetAtk();
            SetCrit();
            ...
            SetName();
            SetCry();
            Console.WriteLine(name + " the Monk\nHp " + hp + "\nAttack " + attack + "\nDefense " + defense + "\nCritChance " + crit + "\nBattlecry " + battlecry);
        }
